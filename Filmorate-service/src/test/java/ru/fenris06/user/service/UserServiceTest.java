...
public class UserServiceTest {
    ...
    public void addFriend_IfUserAndFriendFound_FriendAdd() {
        ...
        verify(userRepository).save(userArgumentCaptor.capture());
        final User testUsers = userArgumentCaptor.getValue();

        assertEquals(testUsers.getId(), user1.getId());


        final List<User> testFriends = new ArrayList<>(testUsers.getFriends());
